...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Build Phase 521-524 IBKR read-only connect dry-run launch pack without connecting, probing, sending, reading account/positions, requesting data, qualifying contracts, or trading.",
    )
...
def main() -> int:
    ...
    if args.ibkr_readonly_connect_dryrun_launch_pack:
        ...
        for field in STATUS_FIELDS:
            print(f"{field}={STATUS_VALUES[field]}")
        print(f"checks={len(rows)}")
        print("csv=operator_ibkr_readonly_connect_dryrun_launch_pack.csv")
        print("report=reports/operator_ibkr_readonly_connect_dryrun_launch_pack_report.md")
        print(
            "NOTICE: Phase 521-524 artifact-only dry-run launch packet. "
            "No IBKR/TWS/Gateway connection, no network probe, no market data request, "
            "no account reads, no position reads, no historical data request, no contract qualification, "
            "no orders, no cancellation, no rebalance, no Telegram real send, "
            "and no execute command is included."
        )
        return 0
