# Phase 81-84 Acceptance Runtime Efficiency

## 1. Purpose

This document defines runtime-efficiency rules for the local acceptance suite.
...
The project acceptance suite previously ran unit tests once at suite level and again inside the daily research run.

This caused duplicate full test execution during local acceptance.

## 3. Change

`daily_research_run.sh` supports:

```bash
./scripts/daily_research_run.sh --skip-tests
cat > scripts/phase81_84_acceptance_runtime_efficiency_check.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
