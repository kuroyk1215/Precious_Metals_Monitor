...
def _gate_rows():
    resolver_rows = _resolver_rows()
    review_rows = build_ibkr_readonly_qualification_candidate_review_pack_rows(resolver_rows, TZ)
    return build_ibkr_readonly_qualification_candidate_final_gate_rows(
        resolver_rows,
        review_rows,
        TZ,
    )
...
def test_candidate_final_gate_defaults_to_closed():
    rows = _gate_rows()
    ...
    assert by_layer["12A"].candidate_count == "4"
    assert by_layer["12A"].excluded_count == "3"
    assert by_layer["12B"].review_required_count == "7"
    assert by_layer["FINAL"].candidate_final_gate_status == "CLOSED"
    assert by_layer["FINAL"].candidate_count == "4"
    assert by_layer["FINAL"].review_required_count == "7"
    assert by_layer["FINAL"].excluded_count == "3"

    for row in rows:
        assert row.candidate_final_gate_status == "CLOSED"
...
def test_candidate_final_gate_detects_unsafe_true_permission():
    ...
    by_layer = {row.layer_id: row for row in rows}
    assert by_layer["12A"].candidate_final_gate_status == "CLOSED"
    assert "unsafe_true_permission_detected" in by_layer["12A"].blocking_summary


def test_candidate_final_gate_writers(tmp_path: Path):
    rows = _gate_rows()

    csv_path = tmp_path / "ibkr_readonly_qualification_candidate_final_gate.csv"
    md_path = tmp_path / "ibkr_readonly_qualification_candidate_final_gate_report.md"

    write_ibkr_readonly_qualification_candidate_final_gate_csv(csv_path, rows)
    write_ibkr_readonly_qualification_candidate_final_gate_report(md_path, rows, "config.yaml")

    csv_text = csv_path.read_text(encoding="utf-8")
    md_text = md_path.read_text(encoding="utf-8")

    assert "candidate_final_gate_status" in csv_text
    assert "CLOSED" in csv_text
    assert "Phase 12C IBKR Read-Only Qualification Candidate Final Gate Report" in md_text
