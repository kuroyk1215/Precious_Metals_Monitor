...
UI_GENERATION = "V5_ICON_TIMELINE_ARTIFACT_POLISH"
LATEST_MAIN_COMMIT = "20fd4fa"
LATEST_MERGED_PR = "221"
...
OUTPUT_CSV = "operator_dashboard_icon_timeline_artifact_polish_pack.csv"
OUTPUT_REPORT = "reports/operator_dashboard_icon_timeline_artifact_polish_pack_report.md"
OUTPUT_PACK = "Precious_Metals_Monitor_Dashboard_Icon_Timeline_Artifact_Polish_Pack.md"
...
.icon-token {
  border-color: rgba(85, 200, 217, 0.45);
  color: var(--cyan);
...
tbody tr:hover td {
  background: rgba(121, 173, 255, 0.045);
}
...
def build_dashboard_html(
    ...
    nav_links = "\n".join(
        f'          <a class="nav-item{" active" if item["label"] == "Dashboard" else ""}" href="{escape(item["href"])}"><span class="nav-icon icon-token">{escape(item["icon_text"])}</span><span>{escape(item["label"])}</span></a>'
...
{nav_links}
      ...
      <div class="content">
        <header class="topbar">
          <div>
            ...
            <div class="badge-bar" aria-label="status token bar">
              ...
              <span class="badge warn">NOT_PRODUCTION</span>
              <span class="badge safe">V5_POLISH</span>
        ...
        <section class="summary-strip" aria-label="summary cards">
          <div class="summary-card"><div class="summary-top"><span class="summary-icon icon-token">SYSTEM</span><span class="label">UI Status</span></div><span class="summary-value mono">{_dd(status["ui_status"])}</span><span class="summary-note">Phase 665-672 icon / timeline / artifact polish</span></div>
          <div class="summary-card"><div class="summary-top"><span class="summary-icon icon-token">BLOCKED</span><span class="label">Market Data</span></div><span class="summary-value mono status-disabled">{_dd(status["market_data_status"])}</span><span class="summary-note">IBKR error code {_dd(status["ibkr_error_code"])}</span></div>
          <div class="summary-card"><div class="summary-top"><span class="summary-icon icon-token">WATCHLIST</span><span class="label">Watchlist</span></div><span class="summary-value mono">{symbols_text}</span><span class="summary-note">US ETF scope only</span></div>
          <div class="summary-card"><div class="summary-top"><span class="summary-icon icon-token">LOCAL</span><span class="label">External Effect</span></div><span class="summary-value mono status-warn">{_dd(status["external_effect"])}</span><span class="summary-note">Local artifact generation only</span></div>
        </section>
...
{_card_header("Operator Timeline", "compact timeline", "5 PHASES", "safe")}
...
def build_report(row: Dict[str, str]) -> str:
    lines = [
        "# Phase 665-672 Dashboard Icon Timeline Artifact Polish Pack Report",
        "",
        "## Status",
        "",
        f"- status={row['status']}",
...
def build_pack(row: Dict[str, str]) -> str:
    lines = [
        "# Precious Metals Monitor Dashboard Icon Timeline Artifact Polish Pack",
        "",
        "## Phase Status",
        "",
        f"- phase={row['phase']}",
...
def generate_dashboard_icon_timeline_artifact_polish_pack(
    *,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build Phase 665-672 dashboard icon timeline artifact polish pack.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print("[DASHBOARD_ICON_TIMELINE_ARTIFACT_POLISH_PACK] generated")
